...
router.get("/singup",estaLogueado, async (req, res) => {
  ...
  if (responsables.length > 0) {
    res.render("auth/singup", { responsables });
  } else {
...
router.post(
  ...
  [
    ...
    check("password")
      ...
      .withMessage("tamaño minimo de 6 a 50 caracteres maximo"),
    check("username")
      .notEmpty()
      .isEmail()
      .isLength({ min: 6, max: 100 })
      .trim()
      .withMessage("tamaño de 6 a 100 caracteres"),
  ],
...
router.post(
  ...
  async (req, res) => {
    ...
    if (errores.array().length > 0) {
      ...
      const validPassword = await helpers.comparePaswsorwd(
        password,
        user.password
      );
      if (validPassword) {
        ...
        await conexion.query("UPDATE   users  set ? WHERE id_usuario= ? ", [
          newDatosUser,
          user.id_usuario,
        ]);
...
router.get("/cambiarestado/:cvu_tecnm", esAdministrador, async (req, res) => {
  ...
  if (cvu_tecnm != req.user.cvu_tecnm) {
    const actualiza = await conexion.query(
      "select * from users where cvu_tecnm=?",
      [cvu_tecnm]
    );

    if (actualiza[0].estado == 1) {
      await conexion.query("UPDATE users SET estado = 0 WHERE cvu_tecnm=?", [
        cvu_tecnm,
      ]);
    } else {
      await conexion.query("UPDATE users SET estado = 1 WHERE cvu_tecnm=?", [
        cvu_tecnm,
      ]);
    }
    ...
    req.flash(
      "message",
      "No esta permitido para este usuario ya que no podra accesar despues"
    );
    res.redirect("/cuentas");
  }
});
