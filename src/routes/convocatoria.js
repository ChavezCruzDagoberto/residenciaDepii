...
router.get("/delete/:id_convocatoria", esAdministrador, async (req, res) => {
  ...
  await conexion.query("DELETE FROM  convocatoria  WHERE ID_CONVOCATORIA=?", [
    id_convocatoria,
  ]);
...
router.get("/edit/:id_convocatoria", esAdministrador, async (req, res) => {
  ...
  const nuevo = await conexion.query(
    "SELECT * FROM  convocatoria  WHERE ID_CONVOCATORIA=?",
    [id_convocatoria]
  );
  //console.log(nuevo[0]);
  ...
  async (req, res) => {
    ...
    if (errores.array().length > 0) {
      ...
      await conexion.query(
        "UPDATE   convocatoria  set ? WHERE ID_CONVOCATORIA= ? ",
        [newConvocatoria, id_convocatoria]
      );
