...
router.post(
  ...
  [
    ...
    check("cvu_tecnm")
      ...
      .toUpperCase()
      .isLength({ max: 10 })
      .withMessage("Solo Alfanumerico con maximo de 10 caracteres"),
    check("nombre")
      .notEmpty()
      .toUpperCase()
      .isLength({ max: 50 })
    ...
    check("apellido1")
      ...
      .isLength({ max: 50 })
      .withMessage("solo Letras"),
    check("plantel_adscripcion")
      .notEmpty()
      .toUpperCase()
      .isLength({ max: 100 })
      .withMessage("solo Alphanumerico"),
    check("email")
      .notEmpty()
      .isEmail()
      .toLowerCase()
  ...
  async (req, res) => {
    ...
    if (errores.array().length > 0) {
      ...
      if (validacion.length > 0) {
        req.flash(
          "message",
          "No se pudo registar ya existe el usuario verifique los datos en caso de ser necesario editelo"
        );
        ...
        if (validacion1.length > 0) {
          req.flash(
            "message",
            "No se pudo registar el correo electronico esta asociado a otro integrante "
          );
...
router.post(
  ...
  [
    ...
    check("cvu_tecnm")
      ...
      .toUpperCase()
      .isLength({ max: 10 })
      .withMessage("Solo Alphanumerico con maximo de 10 caracteres"),
    check("nombre")
      .notEmpty()
      .toUpperCase()
      .isLength({ max: 50 })
    ...
    check("id_proyecto")
      .notEmpty()
      .isNumeric()
      .withMessage("Solo campo numerico"),
  ],
  async (req, res) => {
    const errores = validationResult(req);
    //console.log(errores.array());
...
router.get("/proyecto/:id_proyecto", estaLogueado, async (req, res) => {
  ...
  const integrantes = await pool.query(
    "select * from participante natural join proyecto_participante where id_proyecto= ?",
    [id_proyecto]
  );
...
router.get(
  "/proyecto/delete/:cvu_tecnm/:id_proyecto",
  estaLogueado,
  async (req, res) => {
    const { cvu_tecnm, id_proyecto } = req.params;

    const valida = await pool.query(
      "select * from proyecto_participante where cvu_tecnm= ? and id_proyecto= ?",
      [cvu_tecnm, id_proyecto]
    );
    //console.log(valida[0].rol_proyecto);
    if (valida[0].rol_proyecto == "Responsable") {
      req.flash(
        "message",
        "Usted no se puede eliminar es el responsable del proyecto"
      );
      res.redirect("/integrantes/proyecto/" + id_proyecto);
    } else {
      ...
      await pool.query(
        "delete   from  proyecto_participante where cvu_tecnm= ? and id_proyecto= ?",
        [cvu_tecnm, id_proyecto]
      );
      //console.log(integrantes);
      //res.send("listas tontas");
...
router.get("/activos", esAdministrador, async (req, res) => {
  const integrantes = await pool.query(
    'SELECT * FROM  participante where estado="1" '
  );
...
router.get("/inactivos", esAdministrador, async (req, res) => {
  const integrantes = await pool.query(
    'SELECT * FROM  participante where estado="0" '
  );
...
router.get("/delete/:cvu_tecnm", async (req, res) => {
  ...
  if (cvu_tecnm != req.user.cvu_tecnm) {
    try {
      await pool.query("DELETE FROM  participante  WHERE CVU_TECNM=?", [
        cvu_tecnm,
      ]);
      ...
      req.flash(
        "message",
        "No se ha podido eliminar tiene relacion con otros datos importantes"
      );
      res.redirect("/integrantes");
    }
  } else {
...
router.get("/desactivar/:cvu_tecnm", esAdministrador, async (req, res) => {
  ...
  await pool.query("UPDATE participante SET estado=0 WHERE CVU_TECNM=?", [
    cvu_tecnm,
  ]);
  // console.log(req.params.cvu_tecnm);
  req.flash("success", cvu_tecnm + " Eliminado  correctamente");
  ...
  async (req, res) => {
    ...
    if (errores.array().length > 0) {
      ...
      try {
        ...
        const valicionExiste = await pool.query(
          "select * from participante where cvu_tecnm=?",
          [cvu_tecnm1]
        );
        ...
        if (valicionExiste.length > 0 && cvu_tecnm1 != cvu_tecnm) {
          req.flash(
            "message",
            "Algo ha salido mal usuario existente con clave  " + cvu_tecnm1
          );
          ...
          await pool.query("UPDATE   participante  set ? WHERE CVU_TECNM= ? ", [
            newIntegrante,
            cvu_tecnm,
          ]);
