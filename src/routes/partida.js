...
router.get("/add/:clave_financiamiento", async (req, res) => {
  ...
  const partidas = await conexion.query(
    "select a.clave_partida from (select distinct clave_partida from detalle_partida)as a left join(select clave_partida from financiamiento_partida where clave_financiamiento = ? )as b on a.clave_partida=b.clave_partida where b.clave_partida is null",
    [clave_financiamiento]
  );
  // console.log('partidas', partidas.length);
  if (partidas.length > 0) {
    res.render("partida/add_financiamiento", {
      partidas: partidas,
      clave: clave_financiamiento,
    });
  } else {
...
router.get("/", esAdministrador, async (req, res) => {
  const partidas = await conexion.query(
    "SELECT * FROM  financiamiento_partida "
  );
...
//eliminar
router.get(
  "/delete/:clave_financiamiento/:clave_partida",
  esAdministrador,
  async (req, res) => {
    const { clave_financiamiento } = req.params;
    const { clave_partida } = req.params;
    await conexion.query(
      "DELETE FROM  financiamiento_partida  WHERE CLAVE_FINANCIAMIENTO=? && CLAVE_PARTIDA=?",
      [clave_financiamiento, clave_partida]
    );
...
//editar
router.post(
  "/edit/:clave_financiamiento/:clave_partida",
  [
    check("clave_partida").notEmpty().withMessage("no se acepta vacio"),
    check("monto_aprobado")
      .notEmpty()
      .isNumeric()
      .withMessage("valores enteros"),
  ],
  esAdministrador,
  async (req, res) => {
    ...
    if (errores.array().length > 0) {
      ...
      await conexion.query(
        "UPDATE   financiamiento_partida  set ? WHERE CLAVE_FINANCIAMIENTO= ?  && CLAVE_PARTIDA=?",
        [newPartida, clave_financiamiento, clave_partida]
      );
