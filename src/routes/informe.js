...
router.get("/add", estaLogueado, async (req, res) => {
  ...
  proyecto = await conexion.query(
    "SELECT * FROM proyecto natural join proyecto_participante where CVU_TECNM= ? and ESTADO>=1   ",
    [cvu_tecnm]
  );

  //console.log(proyecto);
  if (proyecto.length > 0) {
    var no_informe = await conexion.query(
      "select * from informe where id_proyecto=?",
      [proyecto[0].id_proyecto]
    );
    if (no_informe.length > 0) {
      ...
      res.render("proyecto/informe/add", {
        proyecto,
        message: req.flash("message"),
      });
    }
  } else {
    res.send("usted no tiene ningun proyecto activo");
  }
});
...
router.post("/listarinforme", estaLogueado, async (req, res) => {
  ...
  let consulta = await conexion.query(
    "select * from informe where ID_PROYECTO=?",
    [id_proyecto]
  );

  res.render("proyecto/informe/listarporproyecto", {
    consulta,
...
router.get("/mostrar/:id_proyecto", estaLogueado, async (req, res) => {
  ...
  for (const aux in ids_informes) {
    var temporal = await conexion.query(
      "select * from archivo_informes where id_informe=? and id_proyecto=?",
      [ids_informes[aux], id_proyecto]
    );
  ...
  if (req.user.rol_sistema == "Administrador") {
    ...
    res.render("proyecto/informe/listarProyectoResponsable", {
      consulta: editado,
    });
  }
  // console.log(editado);
...
router.get("/delete/:id_informe", async (req, res) => {
  ...
  valor = await conexion.query("select * from informe where id_informe= ?", [
    id_informe,
  ]);
  await conexion.query("DELETE FROM  informe  WHERE ID_INFORME=?", [
    id_informe,
  ]);
...
router.post("/edit/:id_informe", async (req, res) => {
  ...
  await conexion.query("UPDATE   informe  set ? WHERE id_informe = ? ", [
    newInforme,
    id_informe,
  ]);
  // console.log(cvu_tecnm);
...
router.get("/cargar/:id_informe", async (req, res) => {
  const { id_informe } = req.params;
  try {
    var extraerProyecto = await conexion.query(
    ...
    extraerProyecto = await conexion.query(
    ...
    const verenArchivos = await conexion.query(
      "select * from archivo_informes where id_informe=? and id_proyecto=?",
      [id_informe, id_pro]
    );
    ...
    switch (no_informe) {
      case 1:
        if (estado >= 3) {
          res.render("proyecto/informe/cargarArchivoInformes", {
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (consulta.length > 0) {
        ...
        var existe_base = await conexion.query(
          "select * from archivo_informes where id_informe=? and id_proyecto=?",
          [id_informe, id_proyecto]
        );
        if (existe_base.length > 0) {
          ...
          await conexion.query(
            ...
            [{ estado: estado_mandar }, id_proyecto]
          );
...
router.get("/verInforme/:id_informe/:id_proyecto", async (req, res) => {
  ...
  res.render("proyecto/informe/verArchivoInforme", {
    informe: resultado[0],
    id_proyecto,
  });
  //res.send('enviado')
...
router.get("/leerInforme/:id_proyecto/:id_informe", async (req, res) => {
  ...
  const resultado = await conexion.query(
    "select url_archivo from archivo_informes where id_proyecto= ? and id_informe=?",
    [id_proyecto, id_informe]
  );
...
router.get("/observaciones/:id_proyecto/:id_informe", async (req, res) => {
  ...
  var validacion = await conexion.query(
    "select * from archivo_informes where id_proyecto=? and id_informe=?",
    [id_proyecto, id_informe]
  );

  res.render("proyecto/informe/observacionesInforme", {
    id_proyecto,
    id_informe,
