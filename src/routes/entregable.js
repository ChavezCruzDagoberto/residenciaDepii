...
router.get("/add/:id_proyecto", estaLogueado, async (req, res) => {
  ...
  if (validaprotocolo.length > 0) {
    const validacion = await conexion.query(
      "select entregable.id_entregable,nombre,contribucion from entregable left join (select * from proyecto_entregable where id_proyecto=?)as a on entregable.id_entregable =a.id_entregable where a.id_entregable is null ",
      [id_proyecto]
    );
    res.render("proyecto/entregable/add", {
      entregables: validacion,
      id_proyecto,
    });
...
router.post("/add", estaLogueado, async (req, res) => {
  ...
  for (const p in id_entregable) {
    ...
    await conexion.query("INSERT INTO proyecto_entregable set ?", [
      newProyecto_entregable,
    ]);
  }
...
router.get("/proyecto/:id_proyecto", estaLogueado, async (req, res) => {
  ...
  const consulta = await conexion.query(
    "select * from entregable natural join proyecto_entregable  where id_proyecto=?",
    [id_proyecto]
  );

  res.render("proyecto/entregable/listarporproyecto", {
    consulta,
    id_proyecto,
  });
});
...
router.post("/edit", async (req, res) => {
  ...
  await conexion.query(
    "UPDATE   proyecto_entregable  set ? WHERE id_proyecto= ? and id_entregable=? ",
    [updateEntregable, id_proyecto, id_entregable]
  );
...
router.get("/delete/:id_entregable/:id_proyecto", async (req, res) => {
  ...
  await conexion.query(
    "DELETE FROM  proyecto_entregable  WHERE id_proyecto= ? and id_entregable=? ",
    [id_proyecto, id_entregable]
  );
