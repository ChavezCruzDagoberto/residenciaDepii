...
passReqToCallback: true //para pásar otro dato extra
    ...
    const rows=await conexion.query('select * from users where username =?',[username]);
        ...
        }else{return done(null,false,req.flash('message','nombre de usuario no existe'));}
}));
...
passReqToCallback:true
     ...
     //extrae de req.body el fullname
...
newUser.password=  await helpers.encriptarPassword(password);
  ...
  const result= await  conexion.query('insert into users set?',[newUser]);
   ...
   return  done(null,null);// done(null, newUser);
        }


 //laugeminis2017@gmail.com
